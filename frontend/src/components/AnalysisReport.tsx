...
function AnalysisReport({ result, analysisId }: AnalysisReportProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900/50 py-4 sm:py-6 lg:py-8 transition-colors">
      <div className="max-w-4xl mx-auto px-4 sm:px-5 lg:px-6">
        <div className="rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/80 p-4 sm:p-6 mb-4 sm:mb-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight leading-tight">
                분석 리포트
              </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className={`rounded-xl border-2 px-4 py-4 sm:px-5 sm:py-5 ${colors.border} ${colors.bg}`}>
              <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">종합 점수</div>
              <div className="flex items-baseline gap-1.5 mt-1.5">
                <span className={`text-2xl sm:text-3xl font-bold tabular-nums ${colors.text}`}>{overallScore}</span>
                <span className="text-sm text-gray-500 dark:text-gray-400">/ 100</span>
              </div>
              <div className={`text-sm font-semibold mt-1.5 ${colors.text}`}>{getScoreLabel(overallScore)}</div>
            </div>
            {highPriorityRecs.length > 0 ? (
              <div className="rounded-xl border-2 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-4 sm:px-5 sm:py-5">
                <div className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider">긴급 개선</div>
                <div className="text-2xl sm:text-3xl font-bold tabular-nums text-red-600 dark:text-red-400 mt-1.5">{highPriorityRecs.length}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">개 항목</div>
              </div>
            ) : (
              <div className="rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 px-4 py-4 sm:px-5 sm:py-5">
                <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">긴급 개선</div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">없음</div>
              </div>
            )}
          </div>
        </div>

        {/* 핵심 지표 카드 - 모바일 1열, 태블릿 2열, 데스크톱 4열 */}
        {product_analysis && (
        ...
        {/* 분석 결과 탭 - 모바일 터치 친화, 가독성 강화 */}
        <div className="rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/80 p-4 sm:p-6 shadow-sm">
          <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100 mb-4 tracking-tight">분석 결과</h2>
          <div className="flex gap-0.5 sm:gap-1 border-b border-gray-200 dark:border-gray-600 mb-5 sm:mb-6 -mx-1 overflow-x-auto">
            <button
              onClick={() => setActiveTab('recommendations')}
              className={`flex-1 min-w-0 px-3 sm:px-4 py-3 sm:py-3.5 text-sm font-semibold rounded-t-lg transition-colors touch-manipulation ${
            ...
            {activeTab === 'recommendations' && (
              <div>
                ...
                {highPriorityRecs.length > 0 && (
                  <section className="mb-6 sm:mb-8">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-500 flex-shrink-0" /> High · {highPriorityRecs.length}건
                    </h3>
                ...
                {mediumPriorityRecs.length > 0 && (
                  <section className="mb-6 sm:mb-8">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500 flex-shrink-0" /> Medium · {mediumPriorityRecs.length}건
                    </h3>
                    <div className="space-y-3 sm:space-y-4">
                ...
                {lowPriorityRecs.length > 0 && (
                  <section>
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-gray-400 flex-shrink-0" /> Low · {lowPriorityRecs.length}건
                    </h3>
                    <div className="space-y-3 sm:space-y-4">
                ...
                {recommendations.length === 0 && (
                  <p className="text-center py-8 sm:py-10 text-gray-500 dark:text-gray-400 text-sm sm:text-base leading-relaxed">개선 제안이 없습니다.</p>
                )}
              </div>
            )}
        ...
        {analysisId && (
          <div className="mt-4 sm:mt-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/80 p-4 sm:p-6 shadow-sm">
            <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100 mb-1 tracking-tight">리포트 다운로드</h2>
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 mb-4 leading-relaxed">PDF, Excel, Markdown 형식으로 저장할 수 있습니다.</p>
