#!/usr/bin/env python3
"""Shop 페이지 분석 테스트 스크립트 (whippedofficial)"""
import requests
import json
import time
import sys
...
def test_shop_analysis():
    ...
    print("=" * 70)
    print("Shop 페이지 분석 테스트 - whippedofficial")
    print("=" * 70)
    print(f"URL: {url}\n")
    ...
    # 2. 분석 결과 대기 및 조회
    print("2️⃣ 분석 결과 대기 중...")
    max_attempts = 90  # 최대 3분 대기
    last_status = None
    last_stage = None
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(f"   ⚠️ 조회 타임아웃 (시도 {attempt}/{max_attempts}), 계속 대기...")
            time.sleep(2)
        except Exception as e:
            print(f"   ⚠️ 조회 중 오류: {e}")
            time.sleep(2)
    ...
    try:
        ...
        if checklist:
            ...
            if items:
                print(f"   - 상세 항목 (최대 10개):")
                for i, item in enumerate(items[:10], 1):
                    status = "✅" if item.get("passed") else "❌"
                    print(f"     {i}. {status} {item.get('title', 'N/A')}")
